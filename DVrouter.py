...
class DVrouter(Router):
    ...
    def __init__(self, addr, heartbeat_time):
        ...
        self.logger.setLevel(logging.INFO)  # Đặt mức log là INFO để ghi các sự kiện quan trọng
        file_handler = logging.FileHandler(f"dvrouter_{addr}.log", mode='w')  # Tạo file log, ghi đè mỗi lần chạy
        formatter = logging.Formatter('[%(asctime)s] [%(name)s] %(message)s')  # Định dạng log: thời gian, tên, thông điệp
        file_handler.setFormatter(formatter)  # Áp dụng định dạng cho file log
        self.logger.addHandler(file_handler)  # Thêm file handler vào logger
        self.logger.info(f"Khởi tạo router {addr}")  # Ghi log khởi tạo router
